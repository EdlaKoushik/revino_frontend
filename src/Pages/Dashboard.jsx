...
const Dashboard = () => {
  ...
  // Remove raw success message and show toast instead
  useEffect(() => {
    if (scheduled) {
          ...
          <main className="flex-1">
            ...
            <div className="max-w-3xl mx-auto mt-10">
              <div className="bg-white rounded-xl shadow p-6 flex items-center justify-between mb-8">
                ...
                >
                  <img src={plusImg} alt="plus" className="w-5 h-5 object-contain" />
                  New Interview
                </button>
              </div>
            </div>
            ...
            {/* Schedule Next Mock button */}
            <div className="flex justify-end mb-6">
              <button
                className="bg-[#6c47ff] text-white font-bold px-6 py-2 rounded-lg shadow hover:bg-[#4f2fcf] transition"
                onClick={() => navigate('/schedule-mock')}
              >
                Schedule Next Mock
              </button>
            </div>
            {/* Scheduled Mocks Section */}
            <div className="max-w-6xl mx-auto mt-10">
              <div className="flex items-center gap-2 text-[#6c47ff] font-semibold text-lg mb-4 mt-2">
                <FaCalendarAlt />
                Scheduled Mocks
              </div>
              <ScheduledMocksList refresh={scheduled} onRefreshed={() => setScheduled(false)} />
            </div>
          </main>
