...
const ScheduleMockPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-[#f7f7fb] p-6">
      ...
      <div className="bg-white rounded-xl shadow p-8 max-w-2xl w-full mt-16">
        <h2 className="text-3xl font-bold text-[#6c47ff] mb-6 text-center">Schedule Next Mock Interview</h2>
        <form onSubmit={handleSchedule} className="space-y-6">
          <div>
            <label className="block font-semibold mb-1">Interview Mode</label>
            <div className="flex gap-4">
              <button type="button" className={`px-4 py-2 rounded-lg border-2 ${mode==='text'?'border-[#6c47ff] bg-[#f3f0ff]':'border-gray-200 bg-white'}`} onClick={()=>setMode('text')}>Text</button>
              <button type="button" className={`px-4 py-2 rounded-lg border-2 ${mode==='audio'?'border-[#6c47ff] bg-[#f3f0ff]':'border-gray-200 bg-white'}`} onClick={()=>setMode('audio')}>Audio</button>
              <button type="button" className={`px-4 py-2 rounded-lg border-2 ${mode==='video'?'border-[#6c47ff] bg-[#f3f0ff]':'border-gray-200 bg-white'}`} onClick={()=>setMode('video')}>Video</button>
            </div>
          </div>
          <div>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              ...
              <textarea className="w-full border rounded-lg px-3 py-2 min-h-[120px] bg-[#fafbff] placeholder:text-gray-400" value={jobDesc} onChange={e=>setJobDesc(e.target.value)} placeholder="Paste the job description here..." />
            </div>
          </div>
          <div>
          ...
          <button type="submit" className="w-full bg-[#6c47ff] text-white font-bold px-6 py-3 rounded-lg shadow hover:bg-[#4f2fcf] transition mt-4" disabled={loading}>
